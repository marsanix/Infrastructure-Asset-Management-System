...
interface Location { id: number; name: string; description: string | null; is_active: boolean }
...
const crud       = useCrud<Location>('/locations')
...
<template>
  <MasterListView ref="listRef" title="Locations" endpoint="/locations"
    permission-module="location" :columns="columns"
    :on-create-click="openCreate" :on-edit-click="openEdit" />

  <MasterFormModal :open="modal" :title="editTarget ? 'Edit Location' : 'Create Location'"
    :submitting="submitting || crud.loading.value"
    @close="modal = false" @submit="handleSubmit(onSubmit)">
    <CInput :model-value="values.name || ''" label="Name" required :error="errors.name"
      autocomplete="off" @update:model-value="setField('name', $event as string)" />
    <CInput :model-value="values.description || ''" label="Description" :error="errors.description"
